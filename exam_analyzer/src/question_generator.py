...
def generate_mock_exam(db_path: str, api_url: str, api_key: str,
                       student_id: str = "", question_count: int = 10,
                       debug_callback=None) -> list[dict]:
    """Generate a mock exam matching topic frequency distribution."""
    db = QADatabase(db_path)
    kps = db.get_all_kps()
    if not kps:
        db.close()
    ...
    # Select KPs weighted by evidence_count (exam frequency)
    weights = [kp.get("evidence_count", 1) for kp in kps]
    ...
    selected = random.choices(kps, weights=weights, k=min(question_count, len(kps)))
    ...
    for kp in selected:
        if kp["id"] not in seen:
            seen.add(kp["id"])
            unique_selected.append(kp)

    questions = []
    difficulties = ["basic", "intermediate", "intermediate", "advanced"]  # 1:2:1 ratio
    for i, kp in enumerate(unique_selected[:question_count]):
        diff = difficulties[i % len(difficulties)]
        template = extract_template(db, kp["id"], client, debug_callback)
        stem = generate_variation(template, diff) if template else ""
        if not stem:
            stem = f"Explain: {kp.get('name', 'this topic')}."
        ans_data = generate_answer(stem, kp["id"], db, client, debug_callback)
        questions.append({
            "stem": stem, "answer": ans_data.get("answer", ""),
            "difficulty": diff, "kp_id": kp["id"], "kp_name": kp.get("name", ""),
            "validated": ans_data.get("validated", False),
        })
